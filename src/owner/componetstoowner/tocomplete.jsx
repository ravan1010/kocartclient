...
export const Tocomplete = () => {
  ...
  try {
    setLoading(true);
    const res = await api.get("/api/owner/afterorderprocess", { withCredentials: true });
  ...
  useEffect(() => {
    orderSchema();
      ...
      const Tocomplete = async(id) => {
        try {
            await api.post(`/api/owner/Tocomplete`,{id},
  ...
  >
    ...
    <div>
        <button type="button" className='border px-4 mx-5 text-white bg-green-700 rounded-2xl my-2' onClick={() => Tocomplete(order._id)}>complete</button>
    </div>
    {/* Address */}
    {order.address && (
      <div className="mb-3">
