...
//         await api.post("/api/user/otp",{ otp : otp },
//          { withCredentials: true })
//          .then((res) => {
//           setSuccess(res.data.message)
...
function MARCHENTverify() {
  const navigate = useNavigate();

  useEffect(() => {
    api
      .get("/auth/marchent/cookie") // cookie auto sent
      .then((res) => {
        console.log("User:", res.data.user, res.data.isActive);

        if (res.data.isActive === true) {
          navigate("/adminlandmark/dashboard"); // or wherever
        }else if (res.data.isActive === false) {
          navigate("/admin/info"); // or wherever
        }
      })
      .catch(() => {
        navigate("/");
